...
func (c *Config) ApplyDefaults() {
	...
	if c.Global.Endpoint == "" {
		c.Global.Endpoint = defaults.Endpoint
	}
}
...
func (c *Config) NormalizeAndPrompt() error {
	...
	for i := range c.Fabrics {
		...
		if c.Fabrics[i].URL == "" {
			c.Fabrics[i].URL = input(fmt.Sprintf("NDFC URL for %s:", label))
		}
	...
	for i := range c.Fabrics {
		if c.Fabrics[i].Username == "" {
			label := c.fabricLabel(i)
			c.Fabrics[i].Username = input(fmt.Sprintf("NDFC username for %s:", label))
		}
	}
	...
	for i := range c.Fabrics {
		...
		label := c.fabricLabel(i)
		pw := inputPassword(fmt.Sprintf("NDFC password for %s (%s):", user, label))
...
// GetEndpoint returns the endpoint with fallback to default.
func (f *FabricConfig) GetEndpoint() string {
	if f.Endpoint != "" {
		return f.Endpoint
	}
	return "all" // default
